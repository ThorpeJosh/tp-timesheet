    ...
    def find_executable_location():
        ...
        if 'conda' in sysconfig_scripts_path:
            ...
            if TP_BIN in bins:
                ...
                raise ValueError(f"It appears you are running in a conda env but the {TP_BIN} binary cannot be found in: {sysconfig_scripts_path}")
        ...
        if TP_BIN in os.listdir(prefix_bin_path):
            return os.path.join(prefix_bin_paths, TP_BIN)

        # Search path relative to this file
        current_file_path = os.path.abspath(__file__)
        python_scripts_path = None
        if 'lib' in current_file_path:
            python_scripts_path = os.path.join(current_file_path.split('/lib/')[0], 'bin')
            if TP_BIN in os.listdir(python_scripts_path):
                return os.path.join(python_scripts_path, TP_BIN)


        raise ValueError(f"Could not find the '{TP_BIN}' executable, searched: ",[sysconfig_scripts_path, prefix_bin_path, python_scripts_path])
